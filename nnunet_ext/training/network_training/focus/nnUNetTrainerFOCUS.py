...
from nnunet_ext.network_architecture.generic_UNet_sparse import Generic_UNet_sparse
...
class nnUNetTrainerFOCUS(nnUNetTrainerMultiHead):
    # -- Trains n tasks sequentially using transfer learning -- #
    def __init__(self, split, task, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False, save_interval=5, already_trained_on=None, use_progress=True,
                 identifier=default_plans_identifier, extension='focus', tasks_list_with_char=None,
                 #custom args
                 ...
                 seed: int = 42, dropout_p: float= 0.9, num_features_in_perc: float = 0.05):
        ...
        self.target_type = target_type.DISTILLED_OUTPUT
        self.seed = seed
        ...
        self.date_time = self.extract_datetime_and_suffix('_'.join(output_folder.split('_')))
        ...
        self.dataset_dm = FeatureRehearsalDataset(
            ...
            retention_ratio=num_features_in_perc,
            random_state=self.seed,
        )

    def extract_datetime_and_suffix(self, path_str):
        """
        Extract datetime and everything after it from a complex path.
        For a pattern like: path/to/something_25.11.2024_08:56:53_test1
        Returns: '25.11.2024_08:56:53_test1'

        Args:
            path_str (str): The full path string

        Returns:
            str: The datetime and suffix string, or None if not found
        """
        # Convert to Path object and get the final directory name
        path = Path(path_str)
        final_dir = path.parts[-1]

        # Define the pattern for date_time (DD.MM.YYYY_HH:MM:SS) and capture everything after
        datetime_pattern = r'(\d{2}\.\d{2}\.\d{4}_\d{2}:\d{2}:\d{2}.*$)'

        # Search for the pattern
        match = re.search(datetime_pattern, final_dir)
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False, detach=True, no_loss=False):
        ...
        if not isinstance(self.network, Generic_UNet_sparse):
            self.network.__class__ = Generic_UNet_sparse
            self.network.set_dropout_p(self.dropout_p)
        ...
        if not rehearse and random.random() < 0.5:
            with torch.no_grad():
    ...
    def store_features(self, task):
        ...
        with torch.no_grad():
            ...
            if not hasattr(self, 'feature_rehearsal_dataloader'):
                ...
                dataset = FeatureRehearsalMultiDataset(self.dataset_dm, math.ceil((self.batch_size * 250) / len(self.dataset_dm)))
                dataloader = BalancedFeatureRehearsalDataLoader(dataset, batch_size=int(self.batch_size), num_workers=0, pin_memory=True,
    ...
    def clean_up(self):
        ...
        summary = self.dataset_dm.sparsity_analyzer.get_summary()
        for dataset_id, stats in summary.items():
            self.print_to_log_file(f"\nDataset {dataset_id}:")
            self.print_to_log_file(f"Average sparsity: {stats['sparsity_ratio'] * 100:.2f}%")
            self.print_to_log_file(f"- Negative values: {stats['negative_ratio'] * 100:.2f}%")
            self.print_to_log_file(f"- Zero values: {stats['zero_ratio'] * 100:.2f}%")
            self.print_to_log_file(f"Random samples analyzed (during rehearsal in training): {stats['samples_analyzed']}")
    ...
    def run_training(self, task, output_folder, build_folder=True):
        ...
        if self.tasks_list_with_char[0][0] == task:
            ...
            for folder in ["gt", "features", "predictions", "feature_pkl"]:
                path = join(self.trained_on_path, self.extension, FEATURE_PATH + self.date_time, folder)
                if not os.path.exists(path):
                    os.makedirs(path)
                else:
        ...
        if task == self.tasks_list_with_char[0][-1]:
            self.print_to_log_file(
                "last task. stopping already and do not extract features (in practice should be done).")
            self.clean_up()
            return

...
from nnunet_ext.paths import default_plans_identifier, evaluation_output_dir, preprocessing_output_dir, \
...
class nnUNetTrainerMultiHead(
    nnUNetTrainerV2):  # Inherit default trainer class for 2D, 3D low resolution and 3D full resolution U-Net
    def __init__(self, split, task, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True,
                 ...
                 use_progress=True,
                 identifier=default_plans_identifier, extension='multihead', tasks_list_with_char=None,
        ...
        if already_trained_on is not None:
            ...
            if self.already_trained_on.get(str(self.fold), None) is None:
                self.already_trained_on[str(self.fold)] = {'finished_training_on': list(), 'start_training_on': None,
                                                           'finished_validation_on': list(),
                                                           'used_identifier': self.identifier,
                                                           'prev_trainer': [self.trainer_class_name],
                                                           'val_metrics_should_exist': False,
                                                           'checkpoint_should_exist': False,
                                                           'tasks_at_time_of_checkpoint': list(),
                ...
                keys = ['finished_training_on', 'start_training_on', 'finished_validation_on', 'used_identifier',
                        'prev_trainer', \
        ...
        assert isinstance(tasks_list_with_char, tuple) and isinstance(tasks_list_with_char[0], list) and isinstance(
            tasks_list_with_char[1], str), \
        ...
        # -- Storage for MASD values
        self.online_eval_masd = []

        # -- Update self.init_tasks so the storing works properly -- #
        self.init_args = (split, task, plans_file, fold, output_folder, dataset_directory, batch_dice, stage,
                          unpack_data,
                          deterministic, fp16, save_interval, self.already_trained_on, use_progress, identifier,
                          extension,
                          tasks_list_with_char, mixed_precision, save_csv, del_log, use_vit, self.vit_type, version,
                          split_gpu,
        ...
        if self.fold == "all":
            ...
            if not isfile(p_splits_file):
                self.print_to_log_file(
                    "Creating new 5-fold cross-validation split for parameter searching based on already existing split file {}...".format(
                        splits_file))
    ...
    def initialize(self, training=True, force_load_plans=False, num_epochs=500, prev_trainer_path=None,
        ...
        super().initialize(training,
                           force_load_plans)  # --> This updates the corresponding variables automatically since we inherit this class

        # -- Set nr_epochs to provided number -- #
        self.max_num_epochs = num_epochs
        # self.optimizer = torch.optim.NAdam(self.network.parameters(), self.initial_lr, weight_decay=self.weight_decay)
    ...
    def initialize_network(self):
        ...
        if self.trainer_path is None:
            ...
            self.process_plans(load_pickle(join(preprocessing_output_dir, self.tasks_list_with_char[0][0],
            ...
            if self.use_vit:
                ...
                nnViTUNetTrainer.initialize_network(
                    self)  # --> This updates the corresponding variables automatically since we inherit this class
                self.mh_network = MultiHead_Module(Generic_ViT_UNet, self.split, self.tasks_list_with_char[0][0],
                                                   ...
                                                   patch_size=patch_size.tolist(), vit_version=self.version,
                                                   vit_type=self.vit_type, \
                                                   split_gpu=self.split_gpu,
                                                   ViT_task_specific_ln=self.ViT_task_specific_ln, \
                                                   first_task_name=self.tasks_list_with_char[0][0], do_LSA=self.LSA,
                                                   do_SPT=self.SPT)
            else:
                ...
                self.mh_network = MultiHead_Module(Generic_UNet, self.split, self.tasks_list_with_char[0][0],
                                                   prev_trainer=self.network,
                                                   input_channels=self.num_input_channels,
                                                   base_num_features=self.base_num_features, \
                                                   num_classes=self.num_classes,
            ...
            self._update_loss_after_plans_change(net_num_pool_op_kernel_sizes,
                                                 patch_size)  # patch_size etc. is restored in here
        ...
        # -- Update the loss so there will be no error during forward function -- #
        self._update_loss_after_plans_change(self.trainer_model.net_num_pool_op_kernel_sizes,
                                             self.trainer_model.patch_size)
        ...
        assert isinstance(self.trainer_model, (nnUNetTrainerV2,
                                               nnUNetTrainerMultiHead)), "The trainer needs to be nnUNetTrainerV2 or nnUNetTrainerMultiHead.."
        ...
        if self.trainer_model.__class__.__name__ == nnUNetTrainerV2.__name__:  # Important when doing evaluation, since nnUNetTrainerV2 has no mh_network
            self.mh_network = MultiHead_Module(Generic_UNet, self.split, self.tasks_list_with_char[0][0],
                                               prev_trainer=self.trainer_model.network,
                                               input_channels=self.num_input_channels,
                                               base_num_features=self.base_num_features, \
                                               num_classes=self.num_classes,
                                               num_pool=len(self.net_num_pool_op_kernel_sizes))
            ...
            self.mh_network = MultiHead_Module(Generic_ViT_UNet, self.split, self.tasks_list_with_char[0][0],
                                               ...
                                               num_classes=self.num_classes,
                                               num_pool=len(self.net_num_pool_op_kernel_sizes), \
                                               patch_size=self.patch_size.tolist(), vit_version=self.version,
                                               vit_type=self.vit_type, \
        ...
        if not self.trainer_model.__class__.__name__ == nnUNetTrainerV2.__name__ \
            ...
            assert self.mh_network.split == prev_split, \
                "To continue training on the fold {} the same split, ie. \'{}\' needs to be provided, not \'{}\'.".format(
                    self.fold, prev_split, self.mh_network.split)
    ...
    def _perform_validation(self, use_tasks=None, use_head=None, call_for_eval=False, param_search=False,
        ...
        if call_for_eval and not param_search:
            assert join(*evaluation_output_dir.split(os.path.sep)[
                         :-1]) in self.output_folder, "You want to perform an evaluation but the output folder does not represent the one for Evaluation results.."
        ...
        nr_batches = self.num_val_batches_per_epoch * 3 if batch_backup % 2 == 0 else self.num_val_batches_per_epoch * 4
        ...
        for idx, task in enumerate(tasks):
            ...
            plans_file, _, self.dataset_directory, _, stage, \
                _ = get_default_configuration(self.network_name, task, running_task,
                                              trained_on_folds['prev_trainer'][idx], \
                                              self.tasks_joined_name, self.identifier, extension_type=self.extension)
            ...
            with torch.no_grad():
                ...
                if use_all_data:
                    for gen in [self.tr_gen, self.val_gen]:
                        ...
                        gen_copy = tee(gen, 1)[
                            0]  # <-- Duplicate the generator so the names are extracted correctly during the loop
                    ...
                    # -- Run an iteration for each batch in validation generator -- #
                    val_gen_copy = tee(self.val_gen, 1)[
                        0]  # <-- Duplicate the generator so the names are extracted correctly during the loop
        ...
        if self.csv:
            ...
            val_res = nestedDictToFlatTable(self.validation_results,
                                            ['Epoch', 'Task', 'subject_id', 'seg_mask', 'metric', 'value'])
        ...
        if call_for_eval and join(*evaluation_output_dir.split(os.path.sep)[:-1]) in self.trained_on_path:
            try:
                ...
                meta_path = join(os.path.sep, *self.trained_on_path.split(os.path.sep)[
                                               :self.trained_on_path.split(os.path.sep).index('metadata') + 1])
    ...
    def get_basic_generators(self, use_all_data=False):
        ...
        if self.threeD:
            dl_tr = DataLoader3D(self.dataset_tr,
                                 self.basic_generator_patch_size if not use_all_data else self.patch_size,
                                 self.patch_size, self.batch_size, False,
            ...
            dl_tr = DataLoader2DWithSlices(self.dataset_tr,
                                           self.basic_generator_patch_size if not use_all_data else self.patch_size,
                                           self.patch_size, self.batch_size,
                                           oversample_foreground_percent=self.oversample_foreground_percent,
                                           pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
            dl_val = DataLoader2DWithSlices(self.dataset_val, self.patch_size, self.patch_size, self.batch_size,
                                            oversample_foreground_percent=self.oversample_foreground_percent,
                                            pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')

        return dl_tr, dl_val

    def run_online_evaluation(self, output, target, properties=None, slice_indices=None):
        """Modified version to track slice indices for proper MASD aggregation"""
        if self.eval_batch:
            ...
            with torch.no_grad():
                # Standard Dice/IoU metrics calculation
                ...
                # Convert to numpy for MASD calculation
                output_seg = output_seg.detach().cpu().numpy()
                target = target.detach().cpu().numpy()

                # Calculate MASD for each sample in batch
                masd_values = []
                for i in range(output_seg.shape[0]):
                    masd_per_class = {}
                    for c in range(1, num_classes):  # Skip background
                        pred_mask = output_seg[i] == c
                        target_mask = target[i] == c
                        spacing = properties[i]['spacing_after_resampling'] if properties is not None else None

                        if pred_mask.ndim == 3:  # For 3D data
                            masd = compute_masd_for_slices(pred_mask, target_mask, spacing)
                        else:  # For 2D data
                            if spacing is not None:
                                spacing = spacing[1:]
                            masd = compute_masd_per_batch(pred_mask, target_mask, spacing)

                        # Store slice index along with MASD results
                        slice_idx = slice_indices[i] if slice_indices is not None else None
                        masd['slice_index'] = slice_idx
                        masd_per_class[f'class_{c}'] = masd

                    masd_values.append(masd_per_class)
                self.online_eval_masd.append(masd_values)

    def finish_online_evaluation_extended(self, task, unique_subject_names=None):
        """Calculate metrics ensuring exact slice counting"""
        ...
        subject_names = unique_subject_names if unique_subject_names is not None else np.unique(self.subject_names_raw)

        # First level: collect all patches per slice
        patch_mapping = {}
        for batch_idx, batch_masd in enumerate(self.online_eval_masd):
            start_idx = batch_idx * len(batch_masd)
            end_idx = start_idx + len(batch_masd)
            batch_subjects = self.subject_names_raw[start_idx:end_idx]

            for sample_idx, (subject, masd_result) in enumerate(zip(batch_subjects, batch_masd)):
                if subject not in patch_mapping:
                    patch_mapping[subject] = {}

                for class_key, class_data in masd_result.items():
                    if class_key not in patch_mapping[subject]:
                        patch_mapping[subject][class_key] = {}

                    slice_idx = class_data['slice_index']
                    if slice_idx is not None:
                        if slice_idx not in patch_mapping[subject][class_key]:
                            patch_mapping[subject][class_key][slice_idx] = []
                        patch_mapping[subject][class_key][slice_idx].append(class_data)

        # Second level: strict majority voting per slice
        masd_subject_map = {}
        for subject in patch_mapping:
            masd_subject_map[subject] = {}

            for class_key in patch_mapping[subject]:
                masd_subject_map[subject][class_key] = {}
                slice_results = patch_mapping[subject][class_key]

                for slice_idx, patch_results in slice_results.items():
                    num_patches = len(patch_results)
                    majority_threshold = num_patches / 2

                    valid_masd_values = [p['MASD'] for p in patch_results if p['MASD'] is not None]
                    num_empty_gt = sum(1 for p in patch_results if p['empty_gt_count'] > 0)
                    num_empty_pred = sum(1 for p in patch_results if p['empty_pred_count'] > 0)
                    num_both_empty = sum(1 for p in patch_results if p.get('both_empty_count', 0) > 0)

                    # Strict categorization - each slice must fall into exactly one category
                    if num_both_empty > majority_threshold:
                        slice_result = {
                            'MASD': None,
                            'empty_gt_count': 1,
                            'empty_pred_count': 1,
                        ...
                        masd_subject_map[subject][class_key][slice_idx] = slice_result
                        continue
                    elif num_empty_gt > majority_threshold:
                        category = 'empty_gt'
                    elif num_empty_pred > majority_threshold:
                        category = 'empty_pred'
                    else:
                        category = 'valid'

                    slice_result = {
                        'MASD': np.mean(valid_masd_values) if valid_masd_values and category == 'valid' else None,
                        'empty_gt_count': 1 if category == 'empty_gt' else 0,
                        'empty_pred_count': 1 if category == 'empty_pred' else 0,
                        'both_empty_count': 1 if category == 'both_empty' else 0,
                        'valid_slice_count': 1 if category == 'valid' else 0
                    }
                    masd_subject_map[subject][class_key][slice_idx] = slice_result

        # Calculate Dice and IoU
        idxs = [np.where(self.subject_names_raw == subject) for subject in subject_names]
        tp = [np.array(self.online_eval_tp)[subject_idxs].sum(axis=0) for subject_idxs in idxs]
        fp = [np.array(self.online_eval_fp)[subject_idxs].sum(axis=0) for subject_idxs in idxs]
        fn = [np.array(self.online_eval_fn)[subject_idxs].sum(axis=0) for subject_idxs in idxs]

        global_iou_per_class_and_subject = []
        global_dc_per_class_and_subject = []
        valid_subjects = []
        valid_idxs = []

        for idx, (i, j, k) in enumerate(zip(tp, fp, fn)):
            if not np.isnan(i).any():
                global_iou_per_class_and_subject.extend([i / (i + j + k)])
                global_dc_per_class_and_subject.extend([2 * i / (2 * i + j + k)])
                valid_subjects.append(subject_names[idx])
                valid_idxs.append(idx)

        # Final aggregation per patient
        patient_masd = {}
        for patient in valid_subjects:
            patient_masd[patient] = {}

            if patient in masd_subject_map:
                for class_idx in range(len(global_iou_per_class_and_subject[0])):
                    class_key = f'class_{class_idx + 1}'
                    if class_key in masd_subject_map[patient]:
                        slice_results = masd_subject_map[patient][class_key]
                        ...
                        for slice_data in slice_results.values():
                            if slice_data['MASD'] is not None:
                                valid_masd_values.append(slice_data['MASD'])
                            total_empty_gt += slice_data['empty_gt_count']
                            total_empty_pred += slice_data['empty_pred_count']
                            total_both_empty += slice_data['both_empty_count']
                            total_valid_slices += slice_data['valid_slice_count']

                        total_slices = len(slice_results)

                        patient_masd[patient][class_idx] = {
                            'MASD': np.mean(valid_masd_values) if valid_masd_values else None,
                            'empty_gt_count': total_empty_gt,
                            'empty_pred_count': total_empty_pred,
                            'both_empty_count': total_both_empty,
        ...
        for idx, subject in enumerate(valid_subjects):
            ...
            for class_label in range(len(global_iou_per_class_and_subject[idx])):
                ...
                if class_label in patient_masd[subject]:
                    masd_data = patient_masd[subject][class_label]
                    if masd_data['MASD'] is not None:
                        metrics_dict['MASD'] = np.float64(masd_data['MASD'])
                    metrics_dict['empty_gt_slices'] = np.float64(masd_data['empty_gt_count'])
                    metrics_dict['empty_pred_slices'] = np.float64(masd_data['empty_pred_count'])
                    metrics_dict['both_empty_slices'] = np.float64(masd_data['both_empty_count'])
                    metrics_dict['valid_slices'] = np.float64(masd_data['valid_slice_count'])
                    metrics_dict['total_slices'] = np.float64(masd_data['total_slices'])

                store_dict[subject][f'mask_{class_label + 1}'] = metrics_dict
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True,
        ...
        assert len(
            trained_on) != 0, "Before performing any validation, the model needs to be trained on at least one task."
    ...
    def update_save_trained_on_json(self, task, finished=True):
        ...
        if finished:  # Task finished with training
            ...
            if self.trainer_model is not None and len(self.already_trained_on[str(self.fold)][
                                                          'prev_trainer']) == 1:  # This is always the case when a pre-trained network is used as initialization
                self.already_trained_on[str(self.fold)]['prev_trainer'][-1:] = [
                    self.trainer_model.__class__.__name__]  # --> The one from the used trainer
                self.already_trained_on[str(self.fold)]['prev_trainer'].append(
                    self.trainer_class_name)  # --> The current trainer we start training with
        ...
        if network_old:
            ...
            if isfile(join(self.output_folder, "model_old.model")):
                ...
                if isfile(join(self.output_folder, "model_best.model")):
                    self.load_checkpoint(join(self.output_folder, "model_best.model"), train, True,
                                         join(self.output_folder, "model_old.model"))
                else:
                    self.print_to_log_file(
            ...
            if self.network_old is not None:  # --> If it does not exist, an error will be thrown, NOTE: "easier to ask for forgiveness than permission" (EAFP) rather than "look before you leap"
                ...
                if isfile(join(self.output_folder, "model_old.model")):
                    if isfile(join(self.output_folder, "model_final_checkpoint.model")):
                        return self.load_checkpoint(join(self.output_folder, "model_final_checkpoint.model"), train,
                                                    True, join(self.output_folder, "model_old.model"))
                    if isfile(join(self.output_folder, "model_latest.model")):
                        return self.load_checkpoint(join(self.output_folder, "model_latest.model"), train, True,
                                                    join(self.output_folder, "model_old.model"))
                    if isfile(join(self.output_folder, "model_best.model")):
    ...
    def load_checkpoint_ram(self, checkpoint, train=True, network_old=False, checkpoint_old=None):
        ...
        self.mh_network.add_n_tasks_and_activate(self.already_trained_on[str(self.fold)]['tasks_at_time_of_checkpoint'],
                                                 self.already_trained_on[str(self.fold)][
                                                     'active_task_at_time_of_checkpoint'])
    ...
    def _build_output_path(self, output_folder, meta_data=False):
        ...
        if self.use_vit:
            ...
            if self.ViT_task_specific_ln:
                ...
                if 'not_task_specific' != output_folder.split(os.path.sep)[
                    -1] and 'not_task_specific' not in output_folder:
            ...
            if Generic_UNet.__name__ != output_folder.split(os.path.sep)[
                -1] and Generic_UNet.__name__ not in output_folder:
                if not meta_data:
        ...
        if 'MH' not in output_folder and 'SEQ' not in output_folder:
            output_folder = os.path.join(output_folder, 'SEQ') if self.transfer_heads else os.path.join(output_folder,
                                                                                                        'MH')
    ...
    def _update_loss_after_plans_change(self, net_num_pool_op_kernel_sizes, patch_size):
        ...
        self.loss = DC_and_CE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False},
                                   {})  # Redefine this since at this stage self.loss is a MultipleOutputLoss2
...
def compute_masd_per_batch(pred: np.ndarray, ref: np.ndarray, spacing=None) -> dict:
    """Compute MASD between prediction and reference masks."""

    def compute_surface_distances(pred_border, ref_border, spacing=None):
        if spacing is None:
            spacing = [1.0] * pred_border.ndim
        ref_dist = ndimage.distance_transform_edt(~ref_border, sampling=spacing)
        pred_dist = ndimage.distance_transform_edt(~pred_border, sampling=spacing)
        return ref_dist[pred_border], pred_dist[ref_border]

    def get_mask_borders(mask):
        return np.logical_xor(mask, binary_erosion(mask))

    # Check empty cases
    is_empty_gt = ref.sum() == 0
    is_empty_pred = pred.sum() == 0
    ...
    # Compute MASD for non-empty case
    pred_border = get_mask_borders(pred > 0)
    ref_border = get_mask_borders(ref > 0)
    pred_distances, ref_distances = compute_surface_distances(pred_border, ref_border, spacing)
    masd = 0.5 * (np.mean(pred_distances) + np.mean(ref_distances))
...
def compute_masd_for_slices(pred: np.ndarray, ref: np.ndarray, spacing=None) -> dict:
    """
    Compute MASD across a stack of 2D slices.
    Args:
        pred: Prediction binary mask (3D array with slices along last dimension)
        ref: Reference binary mask (3D array with slices along last dimension)
        spacing: Pixel spacing
    """
    if pred.ndim != 3 or ref.ndim != 3:
        raise ValueError("Input masks must be 3D arrays")

    masd_values = []
    total_empty_gt = 0
    total_empty_pred = 0
    total_both_empty = 0
    total_slices = pred.shape[0]
    valid_slices = 0
    ...
    for slice_idx in range(total_slices):
        ...
        # Track empty slices
        is_empty_pred = pred_slice.sum() == 0
        is_empty_ref = ref_slice.sum() == 0

        if is_empty_ref and is_empty_pred:
            total_both_empty += 1
            continue
        if is_empty_ref:
            total_empty_gt += 1
        ...
        # Compute MASD for valid slices
        slice_spacing = spacing[:2] if spacing is not None else None
        slice_result = compute_masd_per_batch(pred_slice, ref_slice, slice_spacing)

        if slice_result['MASD'] is not None:
            masd_values.append(slice_result['MASD'])
            valid_slices += 1

    return {
        'MASD': np.mean(masd_values) if masd_values else None,
        'empty_gt_count': total_empty_gt,
        'empty_pred_count': total_empty_pred,
        'both_empty_count': total_both_empty,

...
from nnunet_ext.training.FeatureRehearsalDataset import FeatureRehearsalDataset, FeatureRehearsalTargetType, BalancedFeatureRehearsalDataLoader, FeatureRehearsalMultiDataset, FeatureRehearsalDataLoader
...
class nnUNetTrainerFeatureRehearsal2(nnUNetTrainerMultiHead):
    ...
    def run_training(self, task, output_folder, build_folder=True):
        ...
        if task == self.tasks_list_with_char[0][-1]:
            self.print_to_log_file("last task. stopping already and do not extract features (in practice should be done).")
            self.clean_up()
            return
    ...
    def freeze_network(self):
        ...
        self.network.freeze_conv_weights_only(self.layer_name_for_feature_extraction)
    ...
    def store_features(self, task):
        ...
        with torch.no_grad():
            ...
            dataset = FeatureRehearsalDataset(join(self.trained_on_path, self.extension,  FEATURE_PATH + self.date_time), self.deep_supervision_scales, self.target_type, num_features)
            dataset = FeatureRehearsalMultiDataset(dataset, math.ceil((self.batch_size * 250) / len(dataset)))
            dataloader = FeatureRehearsalDataLoader(dataset, batch_size=int(self.batch_size), num_workers=0, pin_memory=True,
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False, detach=True, no_loss=False):
        ...
        if not isinstance(self.network, Generic_UNet):
            self.network.__class__ = Generic_UNet

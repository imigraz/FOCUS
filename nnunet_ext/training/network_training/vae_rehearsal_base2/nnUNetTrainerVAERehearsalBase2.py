...
from nnunet_ext.training.FeatureRehearsalDataset import FeatureRehearsalConcatDataset, FeatureRehearsalMultiDataset, \
    FeatureRehearsalTargetType, FeatureRehearsalDataLoader, InfiniteIterator
...
class nnUNetTrainerVAERehearsalBase2(nnUNetTrainerLoggingMultiHead):
    ...
    def __init__(self, split, task, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True,
        ...
        super().__init__(split, task, plans_file, fold, output_folder, dataset_directory, batch_dice, stage,
                         ...
                         save_csv, del_log, use_vit, vit_type, version, split_gpu, True, ViT_task_specific_ln, do_LSA,
                         do_SPT,
        ...
        assert self.layer_name_for_feature_extraction.startswith(
            ("conv_blocks_context", "td", "tu", "conv_blocks_localization"))
        ...
        self.init_args = (
        split, task, plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
        ...
        with torch.no_grad():
            ...
            if train:
                ...
                self.print_to_log_file(
                    "the following cases will be used for feature rehearsal training:" + str(case_ids))
                output_folder = join(self.trained_on_path, self.extension, EXTRACTED_FEATURE_PATH_TR)
            else:
                assert set(self.dataset_val.keys()).issubset(set(case_ids)), "idk what, but something is wrong " + str(
                    self.dataset_val.keys()) + " " + str(case_ids)
                case_ids = list(self.dataset_val.keys())
                self.print_to_log_file(
                    "the following cases will be used for feature rehearsal validation:" + str(case_ids))
    ...
    def update_dataloader(self):
        ...
        self.extracted_features_dataset_tr = FeatureRehearsalDataset2(
            join(self.trained_on_path, self.extension, EXTRACTED_FEATURE_PATH_TR), self.deep_supervision_scales,
            FeatureRehearsalTargetType.DISTILLED_OUTPUT,
            num_features, self.tasks_list_with_char[0], load_skips=False, load_meta=True)

        self.extracted_features_dataset_val = FeatureRehearsalDataset2(
            join(self.trained_on_path, self.extension, EXTRACTED_FEATURE_PATH_VAL), self.deep_supervision_scales,
            FeatureRehearsalTargetType.DISTILLED_OUTPUT,
            num_features, self.tasks_list_with_char[0], load_skips=False, load_meta=True)

        dataset = FeatureRehearsalDataset2(join(self.trained_on_path, self.extension, GENERATED_FEATURE_PATH_TR),
                                           self.deep_supervision_scales, FeatureRehearsalTargetType.DISTILLED_OUTPUT,
    ...
    def update_generated_dataloader(self):
        ...
        dataloader = FeatureRehearsalDataLoader(dataset, batch_size=int(self.batch_size), num_workers=2,
                                                pin_memory=True, deep_supervision_scales=self.deep_supervision_scales,
                ...
                try:
                    ...
                    if segs_from_prev_stage[i] is not None:
                        ...
                        assert all([i == j for i, j in
                                    zip(seg_prev.shape, img.shape)]), "image and segmentation from previous " \
                    ...
                    if np.prod(d.shape) + seg_shape > (
                            2e9 / 4 * 0.85):  # *0.85 just to be save, 4 because float32 is 4 bytes
                        print(
    ...
    def clean_up(self,
                 to_be_deleted=[EXTRACTED_FEATURE_PATH_TR, EXTRACTED_FEATURE_PATH_VAL, GENERATED_FEATURE_PATH_TR]):
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
        ...
        return super().predict_preprocessed_data_return_seg_and_softmax(data, do_mirroring, mirror_axes,
                                                                        use_sliding_window, step_size, use_gaussian,
                                                                        pad_border_mode, pad_kwargs, all_in_gpu,
    ...
    def train_vae_main(self):
        ...
        self.print_to_log_file(
            f"using batch size {min(len(self.extracted_features_dataset_tr), max_batch_size)} for feature reahearsal training")
        self.print_to_log_file(
            f"using batch size {min(len(self.extracted_features_dataset_val), max_batch_size)} for feature reahearsal validation")

        extracted_features_train_loader = FeatureRehearsalDataLoader(self.extracted_features_dataset_tr, batch_size=min(
            len(self.extracted_features_dataset_tr), max_batch_size),
                                                                     num_workers=2, pin_memory=True,
                                                                     deep_supervision_scales=self.deep_supervision_scales,
                                                                     persistent_workers=False,
                                                                     shuffle=True)

        extracted_features_val_loader = FeatureRehearsalDataLoader(self.extracted_features_dataset_val, batch_size=min(
            len(self.extracted_features_dataset_val), max_batch_size),
                                                                   num_workers=2, pin_memory=True,
                                                                   deep_supervision_scales=self.deep_supervision_scales,
                                                                   persistent_workers=False,
                                                                   shuffle=False)
    ...
    def train_vae(self, save_callback, max_num_epochs: int, anatomy: str):
        ...
        early_stop = EarlyStop(patience=self.EARLY_STOPPING_PATIENCE[anatomy], save_callback=save_callback,
                               trainer=self, verbose=True)
        ...
        if train_using_rehearsal:
            ...
            dataset = FeatureRehearsalConcatDataset(self.extracted_features_dataset_tr,
                                                    [self.extracted_features_dataset_tr,
                                                     self.generated_feature_rehearsal_dataset])
            dataloader = FeatureRehearsalDataLoader(dataset, batch_size=min(len(self.extracted_features_dataset_tr),
                                                                            max_batch_size),
                                                    num_workers=2, pin_memory=True,
                                                    deep_supervision_scales=self.deep_supervision_scales,
                                                    persistent_workers=False,
                                                    shuffle=True,
                                                    drop_last=len(dataset) % min(
                                                        len(self.extracted_features_dataset_tr), max_batch_size) == 1)
            self.print_to_log_file(
                f"reinitialize dataloader with batch size {min(len(self.extracted_features_dataset_tr), max_batch_size)}")
        else:
            self.print_to_log_file("extracted dataset:", self.extracted_features_dataset_tr.data_patches)
            ...
            if len(self.extracted_features_dataset_tr) < self.feature_rehearsal_dataloader_tr.batch_size * 40:
                dataloader = FeatureRehearsalDataLoader(
                    FeatureRehearsalMultiDataset(self.extracted_features_dataset_tr, 2),
                    self.feature_rehearsal_dataloader_tr.batch_size,
                    num_workers=2, pin_memory=True,
        ...
        for epoch in range(max_num_epochs):
            ...
            self.print_to_log_file(
                f"where {np.mean(reconstruction_losses_tr)} was the reconstruction loss and {np.mean(kl_divergences_tr)} was the KL divergence")
    ...
    def vae_run_iteration(self, data_generator, do_backprop=True, epoch: int = None):
        ...
        if self.fp16:
            with autocast():
                ...
                assert torch.isclose(torch.mean(reconstruction_loss_b),
                                     reconstruction_loss), f"{reconstruction_loss}, {torch.mean(reconstruction_loss_b)}, {reconstruction_loss_b}"
        ...
        if do_backprop:
            ...
            self.plot_vae_progress2(epoch, reconstruction_loss_b, kl_div_b, task_idx, data[0].detach(),
                                    x_hat[0].detach())
    ...
    def generate_features(self, num_samples_per_task=None):
        ...
        for y, task in enumerate(self.already_trained_on[str(self.fold)]['finished_training_on']):
            ...
            l = itertools.cycle(
                itertools.product(self.xs_for_generation, self.ys_for_generation, self.zs_for_generation))
            ...
            for sample_idx, (x, y, z) in tqdm.tqdm(enumerate(l), total=num_samples_per_task):  # 20000
                ...
                output = self.network.feature_forward(features_and_skips)[
                    0]  # <- unpack batch dimension (B,C,H,W) -> (C,H,W)
                ...
                np.save(join(output_folder, task, "features", file_name + "_" + str(num_features - 1) + ".npy"),
                        features.cpu().numpy())
    ...
    def plot_vae_progress2(self, epoch: int, reconstruction_loss_b, kl_div_b, task_idx, sample, reconstruction):
        ...
        if not self.summary_writer.epoch_has_figure():
            ...
            cax = fig.add_axes(
                [axes[1].get_position().x1 + 0.01, axes[1].get_position().y0, 0.02, axes[1].get_position().height])
    ...
    def load_vae(self, path=None):
        ...
        self.initialize_vae(self.get_anatomy(vae_dict['shape']), vae_dict['shape'], vae_dict['num_classes'],
                            vae_dict['conditional_dim'])
        ...
        for _slice in range(d.shape[1]):
            ...
            for x in steps[0]:
                ...
                for y in steps[1]:
                    ...
                    logits, features_and_skips = self.network(data_patch,
                                                              layer_name_for_feature_extraction=self.layer_name_for_feature_extraction)

                    # reconstruct features using vae
                    features_reconstructed, _, _ = self.vae(features_and_skips[-1], task_idx,
                                                            slice_idx_normalized=slice_idx_normalized)
    ...
    def _iterate_over_volume(self, d: np.ndarray, function, segmentation: np.ndarray = None,
                             callback_end_of_slice=None):
        assert len(d.shape) == 4, "data must be c, x, y, z"
        ...
        for _slice in range(d.shape[1]):
            ...
            data_sliced_padded, slicer = pad_nd_image(data_sliced, self.patch_size, 'constant', None, True, None)
            if segmentation is not None:
                segmentation_sliced_padded, _ = pad_nd_image(segmentation_sliced, self.patch_size, 'constant', None,
                                                             True, None)
                assert np.all(segmentation_sliced_padded.shape == data_sliced_padded.shape[
                                                                  1:]), "segmentation_sliced_padded must be of same shape as data_sliced_padded"
            ...
            for x in steps[0]:
                ...
                for y in steps[1]:
                    ...
                    if segmentation is not None:
                        ...
                        assert np.all(
                            segmentation_patch.shape == self.patch_size), "segmentation_patch must be of size patch_size"
    ...
    def ood_detection_by_vae_reconstruction_and_eval_and_build_df(self, d: np.ndarray, segmentation: np.ndarray):
        ...
        for _slice in range(d.shape[1]):
            ...
            aggregated_nb_of_predictions = np.zeros([self.num_classes] + list(data_sliced_padded.shape[1:]),
                                                    dtype=np.float32)
            ...
            for c in range(1, self.num_classes):  # skip background class
                tn, fp, fn, tp = sklearn.metrics.confusion_matrix((segmentation_sliced == c).flatten(),
                                                                  (predicted_segmentation == c).flatten(),
    ...
    def ood_detection_by_segmentation_distortion(self, d: np.ndarray, task_id: int, softmax_normalize: bool):
        ...
        for _slice in range(d.shape[1]):
            ...
            aggregated_results_original = np.zeros([self.num_classes] + list(data_sliced_padded.shape[1:]),
                                                   dtype=np.float32)
            aggregated_results_distorted = np.zeros([self.num_classes] + list(data_sliced_padded.shape[1:]),
                                                    dtype=np.float32)
            aggregated_nb_of_predictions = np.zeros([self.num_classes] + list(data_sliced_padded.shape[1:]),
                                                    dtype=np.float32)
            ...
            for x in steps[0]:
                ...
                for y in steps[1]:
                    ...
                    if torch.cuda.is_available():
                        data_patch = to_cuda(data_patch)
                    logits_original, features_and_skips = self.network(data_patch,
                                                                       layer_name_for_feature_extraction=self.layer_name_for_feature_extraction)

...
# Based on https://github.com/cheng-01037/Causality-Medical-Image-Domain-Generalization/blob/main/models/imagefilter3d.py

class GINGroupConv2D(nn.Module):
    def __init__(self, out_channel=1, in_channel=1, interm_channel=2, scale_pool=[1, 3], n_layer=4, out_norm='frob', init_scale='default', **kwargs):
        ...
        GIN for 2D images
        '''
        super(GINGroupConv2D, self).__init__()
        ...
        self.layers.append(
            GradlessGCReplayNonlinBlock2D(out_channel=interm_channel, in_channel=in_channel, scale_pool=scale_pool, init_scale=init_scale, layer_id=0)
        )
        for ii in range(n_layer - 2):
            self.layers.append(
                GradlessGCReplayNonlinBlock2D(out_channel=interm_channel, in_channel=interm_channel, scale_pool=scale_pool, init_scale=init_scale, layer_id=ii + 1)
            )
        self.layers.append(
            GradlessGCReplayNonlinBlock2D(out_channel=out_channel, in_channel=interm_channel, scale_pool=scale_pool, init_scale=init_scale, layer_id=n_layer - 1, use_act=False)
        )
    ...
    def forward(self, x_in):
        ...
        alphas = torch.rand(b, device=x_in.device)[:, None, None, None]  # b, 1, 1, 1
        alphas = alphas.repeat(1, c, 1, 1)  # b, c, 1, 1
        ...
        if self.out_norm == 'frob':
            in_frob = torch.norm(x_in.view(b, c, -1), dim=(-1, -2), p='fro', keepdim=False)
            in_frob = in_frob[:, None, None, None].repeat(1, c, 1, 1)
            self_frob = torch.norm(mixed.view(b, self.out_channel, -1), dim=(-1, -2), p='fro', keepdim=False)
            self_frob = self_frob[:, None, None, None].repeat(1, self.out_channel, 1, 1)
            mixed = mixed * (1.0 / (self_frob + 1e-5)) * in_frob

        return mixed

class GradlessGCReplayNonlinBlock2D(nn.Module):
    def __init__(self, out_channel=32, in_channel=1, scale_pool=[1, 3], layer_id=0, use_act=True, requires_grad=False, init_scale='default', **kwargs):
        ...
        """
        super(GradlessGCReplayNonlinBlock2D, self).__init__()
    ...
    def forward(self, x_in, requires_grad=False):
        ...
        ker = torch.randn([self.out_channel * b, self.in_channel, k, k], requires_grad=self.requires_grad, device=x_in.device)
        shift = torch.randn([self.out_channel * b, 1, 1], requires_grad=self.requires_grad, device=x_in.device) * 1.0

        x_in = x_in.view(1, b * c, x, y)
        x_conv = F.conv2d(x_in, ker, stride=1, padding=k // 2, dilation=1, groups=b)
        x_conv = x_conv + shift
        if self.use_act:
            x_conv = F.leaky_relu(x_conv)

        x_conv = x_conv.view(b, self.out_channel, x, y)
        return x_conv

...
def run_training(extension='multihead'):
    ...
    parser.add_argument("-val", "--validation_only",
                        help="Use this if you want to only run the validation. This will validate each model "
                             "of the sequential pipeline, so they should have been saved and not deleted.",
                        action="store_true")
    parser.add_argument("-c", "--continue_training",
                        help="Use this if you want to continue a training. The program will determine "
    ...
    parser.add_argument("--exp_name", required=False, default=False,
                        help="name of the experiment. used for every associated folder")
    ...
    parser.add_argument("-t", "--task_ids", nargs="+",
                        help="Specify a list of task ids to train on (ids or names). Each of these "
                             "ids must, have a matching folder 'TaskXXX_' in the raw "
                             "data folder", required=True)
    parser.add_argument("-f", "--folds", nargs="+",
    ...
    if extension in ['ted']:
        parser.add_argument('-ted_lambda', action='store', type=float, nargs=1, required=False, default=1e-3,
                            help='Specify the lambda_d parameter to weight ted knowledge distillation term.'
                                 ' Default: ted_lambda = 1e-3')

    if extension in ['focus']:
        parser.add_argument('-CCD', action='store', type=float, nargs=1, required=False, default=0.9,
                            help='Specify how much sparsity the saved features should have. The value 0.9 means 90% sparsity.'
                                 ' Default: CCD = 0.9')
    ...
    if extension in ['ewc', 'ewc_vit', 'ewc_unet', 'ewc_ln', 'froz_ewc', 'froz_ewc_final', 'ownm1', 'ownm2', 'ownm3',
                     'ownm4']:
    ...
    if extension in ['feature_rehearsal2',  'feature_rehearsal_dm', 'feature_rehearsal_dm_GIN',
                      'feature_rehearsal_no_skips_dm',
                     'feature_rehearsal_no_skips_dm_GIN', 'focus',
                     'feature_rehearsal_no_freeze', 'feature_rehearsal_no_replay',
                     'feature_rehearsal_no_skips']:
        # target_type
        ...
        parser.add_argument("-seed", action='store', required=False, default=42,
                            help="seed used for feature rehearsal")

    if extension in ['feature_rehearsal2',  'feature_rehearsal_dm', 'feature_rehearsal_dm_GIN',
                      'feature_rehearsal_no_skips_dm',
                     'feature_rehearsal_no_skips_dm_GIN',
                      'focus',
                      'feature_rehearsal_no_freeze', 'feature_rehearsal_no_replay',
                     'vae_rehearsal_no_skips',
                     'feature_rehearsal_no_skips', 'vae_rehearsal_no_skips_no_conditioning',
                     'vae_rehearsal_no_skips_larger_vae_force_init',
        ...
        parser.add_argument("-DFP", action='store', required=False, default=0.05, type=float,
                            help="How many slice features should be used in feature rehearsal, 0.0-1.0.")
    ...
    ted_lambda, dropout_p = 1e-3, 0.9
    if extension == 'ted':
        ted_lambda = args.ted_lambda
        if isinstance(args.ted_lambda, list):
            ted_lambda = args.ted_lambda[0]
    if extension == 'focus':
        dropout_p = args.CCD
        if isinstance(args.CCD, list):
            dropout_p = args.CCD[0]
    ...
    if isinstance(split,
                  list):  # When the split get returned as a list, extract the path to avoid later appearing errors
        split = split[0].strip()
    ...
    if isinstance(num_epochs,
                  list):  # When the num_epochs get returned as a list, extract the number to avoid later appearing errors
    ...
    if isinstance(save_interval,
                  list):  # When the save_interval gets returned as a list, extract the number to avoid later appearing errors
    ...
    if extension in ['sequential', 'sequential_GIN',  'plop', 'frozen_nonln', 'frozen_unet',
                     'frozen_vit', 'sequential_no_skips']:
    ...
    if extension in ['ewc', 'ewc_vit', 'ewc_ln', 'ewc_unet', 'froz_ewc', 'froz_ewc_final', 'ownm1', 'ownm2', 'ownm3',
                     'ownm4']:
        # -- Extract ewc_lambda -- #
    ...
    if extension in ['rw']:
        ...
        if continue_training:
            print(
                "Note: It will be continued with previous training, be sure that the provided RW related settings have not "
                "changed from previous one..")
    ...
    if extension == 'lwf':
        ...
        if continue_training:
            print(
                "Note: It will be continued with previous training, be sure that the provided lwf_temperature has not "
                "changed from previous one..")
    ...
    if extension in ['plop', 'pod', 'ownm1', 'ownm2', 'ownm3', 'ownm4']:
        ...
        if continue_training:
            print(
                "Note: It will be continued with previous training, be sure that the provided pod_lambda and pod_scales have not "
                "changed from previous one..")
    ...
    if extension in ['mib', 'ownm1', 'ownm2', 'ownm3']:
        ...
        if continue_training:
            print(
                "Note: It will be continued with previous training, be sure that the provided mib_alpha and mib_lkd have not "
                "changed from previous one..")
    ...
    layer_name_for_feature_extraction = ""
    if extension in ['feature_rehearsal2',  'feature_rehearsal_dm', 'feature_rehearsal_dm_GIN',
                     'feature_rehearsal_no_skips_dm', 'feature_rehearsal_no_skips_dm_GIN',
                      'focus',
                     'feature_rehearsal_no_freeze', 'feature_rehearsal_no_replay', 'feature_rehearsal_no_skips']:
        feature_rehearsal_target_type = FeatureRehearsalTargetType[args.target_type.upper()]

    if extension in ['feature_rehearsal2', 'feature_rehearsal_dm', 'feature_rehearsal_dm_GIN',
                     'feature_rehearsal_no_skips_dm', 'feature_rehearsal_no_skips_dm_GIN', 'focus',
                     'feature_rehearsal_no_freeze',
                     ...
                     'rehearsal_no_skips_frozen', 'curl', 'seg_dist', 'vae_rehearsal_no_skips_large',
                     'vae_rehearsal_base2_pipe',
    ...
    num_features_in_perc = 0.05
    if extension in ['focus']:
        num_features_in_perc = args.DFP
        if isinstance(num_features_in_perc, list):
            num_features_in_perc = num_features_in_perc[0]
    # -------------------------------
    ...
    assert isinstance(fold, (int,
                             list)), "Training multiple tasks in {} mode only uses one or multiple folds specified as integers..".format(
        extension)
    ...
    basic_exts = {'save_interval': save_interval, 'identifier': init_identifier, 'extension': extension,
                  ...
                  'split_gpu': split_gpu, 'transfer_heads': transfer_heads,
                  'ViT_task_specific_ln': ViT_task_specific_ln,
                  'do_LSA': do_LSA, 'do_SPT': do_SPT, **basic_args}
    ted_args = {'lambda_d': ted_lambda, **basic_exts}
    ...
    ownm1_args = {'ewc_lambda': ewc_lambda, 'pod_lambda': pod_lambda, 'pod_scales': pod_scales, 'do_pod': do_pod,
                  'mib_lkd': mib_lkd, 'mib_alpha': mib_alpha, **basic_exts}
    ownm3_args = {'do_LSA': do_LSA, 'do_SPT': do_SPT, **ownm1_args, **basic_exts}
    ownm4_args = {'ewc_lambda': ewc_lambda, 'pod_lambda': pod_lambda, 'pod_scales': pod_scales, 'do_pod': do_pod,
                  'pseudo_alpha': pseudo_alpha, **basic_exts}

    rehearsal_no_skips_frozen_args = {'layer_name_for_feature_extraction': layer_name_for_feature_extraction,
                                      **reh_args}
    # feature rehearsal
    rehearsal_args = {'target_type': feature_rehearsal_target_type,
                      'num_rehearsal_samples_in_perc': num_rehearsal_samples_in_perc,
                      'layer_name_for_feature_extraction': layer_name_for_feature_extraction, 'seed': seed,
                      **basic_exts}
    sparse_rehearsal_args = {'dropout_p': dropout_p, 'num_features_in_perc': num_features_in_perc, **rehearsal_args}
    vae_rehearsal_args = {'num_rehearsal_samples_in_perc': num_rehearsal_samples_in_perc,
                          'layer_name_for_feature_extraction': layer_name_for_feature_extraction, **basic_exts}
    ...
    args_f = {'nnUNetTrainerRW': rw_args, 'nnUNetTrainerMultiHead': basic_exts,
              ...
              'nnUNetTrainerTED': ted_args,
              'nnUNetTrainerSequentialGIN': basic_exts,
              ...
              # 'nnUNetTrainerFrozEWCFinal': ewc_args,
              ...
              'nnUNetTrainerFOCUS': sparse_rehearsal_args,
              'nnUNetTrainerFeatureRehearsal2': rehearsal_args,
              'nnUNetTrainerFeatureRehearsalDynamicMemory': rehearsal_args,
              'nnUNetTrainerFeatureRehearsalNoSkipsDynamicMemory': rehearsal_args,
              'nnUNetTrainerFeatureRehearsalNoSkipsDynamicMemoryGIN': rehearsal_args,
              'nnUNetTrainerFeatureRehearsalDynamicMemoryGIN': rehearsal_args,
        ...
        if continue_training:
            ...
            if use_vit:
                ...
                if args.continue_from_previous_dir:
                    ...
                    base_path = join(network_training_output_dir, network, tasks_excluding_last, 'metadata',
                                     Generic_ViT_UNet.__name__ + 'V' + str(version), vit_type.lower())
                else:
                    base_path = join(network_training_output_dir, network, tasks_joined_name, 'metadata',
                                     Generic_ViT_UNet.__name__ + 'V' + str(version), vit_type.lower())
            ...
            if isinstance(trained_on_folds, dict):
                ...
                running_task_list = already_trained_on[str(t_fold)]['finished_training_on'][
                                    :]  # Without '[:]' reference will change over time as well !
            ...
            if began_with is None:
                ...
                try:
                    if np.array(np.array(all_tasks) == np.array(run_tasks)).all() \
                            and np.array(np.array(all_tasks) == trained_on_folds.get('finished_validation_on', np.array(
                        list()))).all():  # Use numpy because lists return true if at least one match in both lists!
            ...
            if began_with != -1:
                ...
                if extension in ['rehearsal', 'rehearsal_no_skips_frozen']:
                    assert seed == int(trained_on_folds['used_seed']), \
                        "To continue training on the fold {} the same seed, ie. \'{}\' needs to be provided, not \'{}\'.".format(
                            t_fold, trained_on_folds['used_seed'], seed)
                ...
                if extension == 'ewc':
                    assert ewc_lambda == float(trained_on_folds['used_ewc_lambda']), \
                        "To continue training on the fold {} the same ewc_lambda, ie. \'{}\' needs to be provided, not \'{}\'.".format(
                            t_fold, trained_on_folds['used_ewc_lambda'], ewc_lambda)
                ...
                if extension == 'lwf':
                    assert lwf_temperature == float(trained_on_folds['used_lwf_temperature']), \
                        "To continue training on the fold {} the same lwf_temperature, ie. \'{}\' needs to be provided, not \'{}\'.".format(
                            t_fold, trained_on_folds['used_lwf_temperature'], lwf_temperature)
                ...
                if extension == 'plop' or extension == 'pod':
                    assert pod_lambda == float(trained_on_folds['used_pod_lambda']), \
                        "To continue training on the fold {} the same lambda, ie. \'{}\' needs to be provided, not \'{}\'.".format(
                            t_fold, trained_on_folds['used_pod_lambda'], pod_lambda)
                ...
                if extension == 'rw':
                    assert update_fisher_every == int(trained_on_folds['update_fisher_after']), \
                        "To continue training on the fold {} the interval of fisher actualization should ont have changed, ie. \'{}\' needs to be provided, not \'{}\'.".format(
                            t_fold, trained_on_folds['update_fisher_after'], update_fisher_every)
                    assert rw_alpha == float(trained_on_folds['used_alpha']), \
                        "To continue training on the fold {} the same alpha, ie. \'{}\' needs to be provided, not \'{}\'.".format(
                            t_fold, trained_on_folds['used_alpha'], rw_alpha)
                ...
                if extension == 'mib':
                    assert mib_alpha == float(trained_on_folds['used_alpha']), \
                        "To continue training on the fold {} the same alpha, ie. \'{}\' needs to be provided, not \'{}\'.".format(
                            t_fold, trained_on_folds['used_alpha'], seed)
                ...
                print(
                    "Fold {} has not been trained on all tasks --> continue the training with restoring task {}..".format(
                        t_fold, began_with))
        ...
        for idx, t in enumerate(tasks):
            ...
            plans_file, output_folder_name, dataset_directory, batch_dice, stage, \
                trainer_class = get_default_configuration(network, t, running_task, network_trainer, tasks_joined_name, \
                                                          plans_identifier, extension_type=extension, exp_name=exp_name)
            ...
            if idx == 0 and not continue_training:
                ...
                possible_trainers = set(
                    TRAINER_MAP.values())  # set to avoid the double values, since every class is represented twice
                ...
                assert issubclass(trainer_class, TRAINER_MAP[extension]), \
                    ...
                    " So choose {}" \
                    " as a network_trainer corresponding to the network, or use the convential nnunet command to train.".format(
                        TRAINER_MAP[extension], extension, TRAINER_MAP[extension])
            ...
            if idx == 0 and init_seq:
                ...
                assert isinstance(t, str) and prev_trainer is not None and init_identifier is not None and isinstance(
                    t_fold, int), \
                ...
                plans_file, prev_trainer_path, dataset_directory, batch_dice, stage, \
                    trainer_class = get_default_configuration(network, all_tasks[0], running_task, prev_trainer,
                                                              prev_directory, \
                ...
                if trainer_class is None:
                    raise RuntimeError(
                        "Could not find trainer class in nnunet.training.network_training nor nnunet_ext.training.network_training")
            ...
            if idx == 0 or idx == 1 and init_seq:
                ...
                trainer = trainer_class(split, all_tasks[0], plans_file, t_fold, output_folder=output_folder_name,
...
def main_sequential_GIN():
    run_training(extension='sequential_GIN')


def main_ted():
    run_training(extension='ted')
...
def main_feature_rehearsal_dm():
    run_training(extension='feature_rehearsal_dm')


def main_feature_rehearsal_dm_GIN():
    run_training(extension='feature_rehearsal_dm_GIN')


def main_feature_rehearsal_no_skips_dm():
    run_training(extension='feature_rehearsal_no_skips_dm')


def main_feature_rehearsal_no_skips_dm_GIN():
    run_training(extension='feature_rehearsal_no_skips_dm_GIN')

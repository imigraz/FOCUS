...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    assert isinstance(trainer, nnUNetTrainer), "trainer is trainer class"
    q = Queue(1)
    processes = []
...
def predict_cases(params_ext, model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    :return:
    """
    assert len(list_of_lists) == len(output_filenames), "input and output filenames are of different length"
    if segs_from_prev_stage is not None: assert len(segs_from_prev_stage) == len(output_filenames), "len(segs) != len(outputs)"
...
def predict_from_folder(params_ext, model: str, input_folder: str, output_folder: str,
    ...
    """
    Modified to handle plans.pkl location with datetime-stamped experiment folders.
    Example path structure:
    .../Task111_Prostate-BIDMC_Task112_Prostate-I2CVB/nnUNetTrainerFeatureRehearsalDynamicMemoryScores__nnUNetPlansv2.1_12.12.2024_18:28:34_patient_uniform_seed1/Generic_UNet/SEQ
    """
    ...
    if no_load:
        ...
        original_path = os.path.normpath(model)
        splitted_path = original_path.split(os.sep)

        # Find the multi-task directory index (contains multiple Task entries)
        task_dir_idx = None
        for i, part in enumerate(splitted_path):
            if part.count('Task') >= 1:
                task_dir_idx = i
                break

        if task_dir_idx is None:
            raise ValueError("Could not find multi-task directory in path")

        # Get current training task combination (e.g., 'Task111_Prostate-BIDMC_Task112_Prostate-I2CVB')
        current_task_combo = splitted_path[task_dir_idx + 1]
        import re
        tasks = [t.lstrip('_') for t in re.split(r'(?=_Task)', current_task_combo)]
        if not tasks[0].startswith('Task'):
            tasks[0] = 'Task' + tasks[0]

        # Check if we're beyond first task
        is_first_task = len(tasks) == 1

        if is_first_task:
            # For first task, use path as is
            plans_path = original_path
        else:
            # For subsequent tasks, need to find corresponding experiment in first task's folder
            first_task = tasks[0]

            # Build path to first task's directory
            base_path = splitted_path[:task_dir_idx + 1]  # Up to multi-task dir
            base_path.append(first_task)  # Add first task only

            # Get experiment folder name parts
            experiment_folder = splitted_path[task_dir_idx + 2]  # The datetime stamped folder
            trainer_name = experiment_folder.split('__')[0]  # Get base trainer name

            # Find matching experiment folder in first task directory
            experiment_path = '/' + os.path.join(*base_path)
            matching_experiments = []
            try:
                for d in os.listdir(experiment_path):
                    if d.startswith(trainer_name) and '__nnUNetPlans' in d:
                        matching_experiments.append(d)
            except FileNotFoundError:
                raise FileNotFoundError(f"Could not find first task directory at {experiment_path}")

            if not matching_experiments:
                raise FileNotFoundError(f"Could not find matching experiment folder in {experiment_path}")

            # Use the first matching experiment (should only be one)
            experiment_folder = matching_experiments[0]
            # Include Generic_UNet/SEQ in the path
            plans_path = join(experiment_path, experiment_folder, 'Generic_UNet', 'SEQ')

    if copy_plans:
        plans_file = join(plans_path, 'plans.pkl')
        if not os.path.exists(plans_file):
            raise FileNotFoundError(f"Could not find plans.pkl at {plans_file}")
        shutil.copy(plans_file, output_folder)
    ...
    if mode == "normal":
        ...
        return predict_cases(params_ext, model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts],
                             folds,
                             ...
                             disable_postprocessing=disable_postprocessing, no_load=no_load, trainer=trainer,
                             params=params)
    else:
